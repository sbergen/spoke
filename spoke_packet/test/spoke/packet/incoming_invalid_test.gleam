...
pub fn decode_invalid_id_test() {
  ...
  |> should.equal(Error(packet.InvalidPacketIdentifier(0)))
}

pub fn connack_decode_invalid_length_test() {
  incoming.decode_packet(<<2:4, 0:4, 3:8, 0:32>>)
  |> should.equal(Error(packet.InvalidData))
}
...
pub fn ping_resp_too_short_test() {
  let assert Error(packet.DataTooShort) = incoming.decode_packet(<<13:4, 0:4>>)
}

pub fn ping_resp_invalid_length_test() {
  let assert Error(packet.InvalidData) =
...
pub fn pub_xxx_decode_invalid_test() {
  ...
  let assert Error(packet.InvalidData) = incoming.decode_packet(data)
